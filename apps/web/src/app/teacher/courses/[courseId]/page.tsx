...
import { useAddChapter, useAddLesson, useAddLessonVideo, useCourseDetail, useDeleteLessonVideo, useUploadLessonVideo } from '@/lib/api/hooks';
...
export default function TeacherCourseContentPage() {
  ...
  const submitUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file || !uploadLessonId) return;
    uploadVideo.mutate({ lessonId: uploadLessonId, file });
    event.target.value = '';
  };
