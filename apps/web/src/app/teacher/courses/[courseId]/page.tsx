...
export default function TeacherCourseContentPage() {
  ...
  const submitImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file && imageTitle.trim()) uploadResource.mutate({ file, title: imageTitle.trim() }, { onSuccess: () => setImageTitle('') });
    event.target.value = '';
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2"><CardHeader><CardTitle className="text-base">Files and resources</CardTitle></CardHeader><CardContent className="space-y-3"><Input value={resourceTitle} onChange={(event) => setResourceTitle(event.target.value)} placeholder="Resource title" /><input ref={resourceInputRef} className="sr-only" type="file" accept="video/*,.pdf,.doc,.docx,.zip" disabled={!resourceTitle.trim() || uploadResource.isPending} onChange={submitResourceUpload} /><Button type="button" variant="outline" className="w-full" disabled={!resourceTitle.trim() || uploadResource.isPending} onClick={() => resourceInputRef.current?.click()}><Upload className="mr-2 h-4 w-4" />{uploadButtonLabel}</Button><div className="space-y-2">{fileResources.map((resource) => renderResource(resource, false))}{!fileResources.length && <p className="text-muted-foreground text-sm">No files uploaded yet.</p>}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base">Images</CardTitle></CardHeader><CardContent className="space-y-3"><Input value={imageTitle} onChange={(event) => setImageTitle(event.target.value)} placeholder="Image title" /><input ref={imageInputRef} className="sr-only" type="file" accept="image/*" disabled={!imageTitle.trim() || uploadResource.isPending} onChange={submitImageUpload} /><Button type="button" variant="outline" className="w-full" disabled={!imageTitle.trim() || uploadResource.isPending} onClick={() => imageInputRef.current?.click()}><FileImage className="mr-2 h-4 w-4" />{uploadResource.isPending ? 'Uploading...' : 'Choose image'}</Button><div className="space-y-2">{imageResources.map((resource) => renderResource(resource, true))}{!imageResources.length && <p className="text-muted-foreground text-sm">No images uploaded yet.</p>}</div></CardContent></Card>
