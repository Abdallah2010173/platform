...
export default function TeacherCourseContentPage() {
  ...
  const handleUploadedVideo = (files: Array<{ url: string; name?: string }>) => {
    const uploaded = files[0];
    if (!uploaded || !uploadLessonId) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2"><CardHeader><CardTitle className="text-base">{t('Files and resources')}</CardTitle></CardHeader><CardContent className="space-y-3"><Input value={resourceTitle} onChange={(event) => setResourceTitle(event.target.value)} placeholder={t('Resource title (optional)')} /><FileUploadButton endpoint="fileUploader" onClientUploadComplete={(files) => saveUploadedResource(files, 'FILE')} onUploadError={(error) => window.alert(`File upload failed: ${error.message}`)} appearance={{ button: '!bg-primary !text-primary-foreground w-full h-10 rounded-md border border-primary text-sm font-medium', allowedContent: 'hidden' }} content={{ button: t('Choose file from device') }} /><div className="space-y-2">{fileResources.map((resource) => renderResource(resource, false))}{!fileResources.length && <p className="text-muted-foreground text-sm">{t('No files uploaded yet.')}</p>}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base">{t('Images')}</CardTitle></CardHeader><CardContent className="space-y-3"><Input value={imageTitle} onChange={(event) => setImageTitle(event.target.value)} placeholder={t('Image title (optional)')} /><ImageUploadButton endpoint="imageUploader" onClientUploadComplete={(files) => saveUploadedResource(files, 'IMAGE')} onUploadError={(error) => window.alert(`Image upload failed: ${error.message}`)} appearance={{ button: '!bg-primary !text-primary-foreground w-full h-10 rounded-md border border-primary text-sm font-medium', allowedContent: 'hidden' }} content={{ button: t('Choose image') }} /><div className="space-y-2">{imageResources.map((resource) => renderResource(resource, true))}{!imageResources.length && <p className="text-muted-foreground text-sm">{t('No images uploaded yet.')}</p>}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base">{t('Lesson videos')}</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-muted-foreground text-sm">{t('Choose a lesson, then upload its video.')}</p><select className="border-input bg-background h-9 w-full rounded-md border px-3 text-sm" value={uploadLessonId} onChange={(event) => setUploadLessonId(event.target.value)}><option value="">{t('Choose a lesson')}</option>{chapters.flatMap((chapter) => (chapter.lessons ?? []).map((lesson) => <option key={lesson.id} value={lesson.id}>{chapter.title} / {lesson.title}</option>))}</select>{uploadLessonId ? <VideoUploadButton endpoint="videoUploader" onClientUploadComplete={handleUploadedVideo} onUploadError={(error) => window.alert(`Video upload failed: ${error.message}`)} appearance={{ button: '!bg-primary !text-primary-foreground ut-uploading:cursor-not-allowed w-full h-10 rounded-md border border-primary text-sm font-medium', allowedContent: 'hidden' }} content={{ button: t('Choose video') }} /> : <Button type="button" variant="outline" className="w-full" disabled><Video className="mr-2 h-4 w-4" />{t('Choose a lesson first')}</Button>}</CardContent></Card>
